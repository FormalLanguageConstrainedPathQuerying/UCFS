 ...
 */
class Gll<VertexType> private constructor(
    override var ctx: Context<VertexType>,
    private val engine: IIntersectionEngine,
) : IGll<VertexType> {
    companion object {
        /**
        ...
        fun <VertexType> gll(
            startState: RsmState,
            inputGraph: IInputGraph<VertexType>,
        ): Gll<VertexType> {
    ...
    private fun handlePoppedGssEdge(
        poppedGssEdge: GssEdge<VertexType>,
        descriptor: Descriptor<VertexType>,
        childSppf: RangeSppfNode<VertexType>,
    ) {
        val leftRange = poppedGssEdge.matchedRange
        val startRsmState =
            if (poppedGssEdge.matchedRange.type is EmptyType) {
                poppedGssEdge.gssNode.rsm
            } else {
                poppedGssEdge.matchedRange.rsmRange!!.to
            }
        val rightRange =
    ...
    override fun handleDescriptor(descriptor: Descriptor<VertexType>) {
        ...
        if (descriptor.rsmState.isFinal) {
            val matchedRange =
                if (descriptor.sppfNode.type is EmptyType) {
